...
proc buildGUI {} {
	global mainframe pages PDFhandle planPDFhandle commentsarear videoPath annotIDLocation testProcedurePath
	set descmenu {
		"&File" {} {} 0 {
	        {command "&New Project"     {} "Create a new Test Project"     {Ctrl n} -command newProject}
	        {command "&Open Project" {} "Open an existing Project" {Ctrl o} -command openProject}
	        {separator}
	        {command "E&xit" {} "Exit the application" {} }
	    }
	    "&Test" {} {} 0 {
	    ...
	    "&Report" {} {} 0 {
	    	{command "Generate report" {} "Generate report"     {} -command fillReport}
	    }
	   	"&Help" {} {} 0 {
	    }
	}
	...
	#创建评论区
	set commentsarear [listbox $mainframe.comments.commentsbox]
	...
	bind $mainframe <Destroy> {
		...
		# 将数据写入文件
		if {[info exist annotIDLocation]} {
			set filename [file join [pwd] data "annotIDLocation_[lindex [split [file tail $testProcedurePath] .] 0].txt"]
...
proc newProject {} {
	...
	wm title .new "New eTestWitness Project"
	# window is a transient window of window ., make sure grab
	wm transient .new .
	catch {grab .new}

	# window content definition
	ttk::labelframe .new.tf -text "Project"
	set tf .new.tf
	
	ttk::frame $tf.f1
	ttk::label $tf.f1.lb -text "Project name:"
	ttk::entry $tf.f1.e -textvariable tempdata(projectName)
	
	ttk::frame $tf.f2
	ttk::button $tf.f2.b -text "Select Directory" -command "set tempdata(projectDir) \[tk_chooseDirectory\]"
	ttk::entry $tf.f2.e -textvariable tempdata(projectDir)
	
	ttk::frame .new.f3

	ttk::button .new.f3.bok -text Ok -command {
		if {$::tempdata(projectDir) eq "" || $::tempdata(projectName) eq ""} {
			tk_messageBox -title "New Project" -message "Select project directory and name"
		} else {
			destroy .new
			set ::projectdata(projectDir) $::tempdata(projectDir)
			set ::projectdata(projectName) $::tempdata(projectName)
			file mkdir [file join $::tempdata(projectDir) $::tempdata(projectName)]
			foreach dir {input data videos report} {			
				file mkdir [file join $::tempdata(projectDir) $::tempdata(projectName) $dir]
			}
			
		}
	}
	
	ttk::button .new.f3.bcancel -text "Cancel" -command {
		set tempdata(projectDir) ""
		set tempdata(projectName) ""
		destroy .new
	}

	# pack items
	pack $tf.f1.lb -side left -padx 10
	pack $tf.f1.e -side left -padx 10 -fill x -expand yes
	pack $tf.f2.b -side left -anchor n -padx 10 
	pack $tf.f2.e -side left -anchor n -padx 10 -fill x -expand yes
	pack .new.f3.bcancel .new.f3.bok -side right -padx 10 -pady 10
	pack $tf.f1 -fill x -expand no -padx 10 -pady {10 30}
	pack $tf.f2 -fill x -expand no -padx 10 -pady {10 30}
	pack .new.tf -fill both -expand yes -padx 10 -pady 10
	pack .new.f3 -fill x -expand yes
...
proc openProject {} {
	# dialog window
	set reportFiles ""
	if {[set path [tk_chooseDirectory]] eq ""} {
		return;
	}
	
	# check the project directories existence
	foreach dir {input data videos report} {
		 set ${dir}_exists [file exists [file join [file join {*}[lrange [file split $path] 0 end-1]] [lindex [file split $path] end] $dir ]]
	}

	# if the project structure is not correct, raise an error message
	if {!($data_exists && $input_exists && $videos_exists && $report_exists)} {
		tk_messageBox -title [mc "Open eTestWitness Project"] -icon error -message [mc "Malformed project directory or not a project directory"] \
						-detail "[mc {Valid project directory should contain:}] data, input, videos, Report"
		return
	}
	# logic
	if {$path ne ""} {
		...
		foreach reportFile [glob -nocomplain -directory [file join [pwd] report] -type f *] {
			lappend reportFiles [file tail $reportFile]
		}

		foreach inputFile [glob -nocomplain -directory [file join [pwd] input] -type f *] {
			set file [file tail $inputFile]
			if {$file ni $reportFiles && [file extension $file] eq ".pdf"} {
				file copy -force $inputFile [file join [pwd] report $file]
			}
		}
		loadConfiguration
	}
...
proc loadConfiguration {} {
	global confExcelPath reportExcelPath schemePlanPath commentsarear testProcedurePath reportStartCell
	set confExcelPath [file join [pwd] input "config.xlsx"]
	try {
		...
		set ws [Excel GetWorksheetIdByName $wb "file Configure"]
		set ws2 [Excel GetWorksheetIdByName $wb "report Configure"]
		set matrix [Excel GetWorksheetAsMatrix $ws ]
		set matrix2 [Excel GetWorksheetAsMatrix $ws2 ]
		foreach line $matrix {
			if {[lindex $line 0]=="Test Procedure"} {
				set testProcedurePath [file join [pwd] report "[lindex $line 1]"]
				...
				set schemePlanPath [file join [pwd] report "[lindex $line 1]"]
				if {![file exists $schemePlanPath]} {puts "file \"$schemePlanPath\" not exist"; continue}
				openPlanPDF $schemePlanPath
			} elseif {[lindex $line 0]=="Repot"} {
				set reportExcelPath [file join [pwd] input "[lindex $line 1]"]
				if {![file exists $reportExcelPath]} {puts "file \"$reportExcelPath\" not exist"; continue}
			}
		}
		foreach line $matrix2 {
			if {[lindex $line 0]=="Test case summarised"} {
				set reportStartCell [lindex $line 1]
			}
		}
	...
	catch {Excel Close $wb}
	Cawt Destroy $app
}

# 加载图片到excel
proc loadPicture2excel {{picturefile ""}} {
	global reportExcelPath
	
	if {$picturefile== ""} {set picturefile [tk_getOpenFile] }
	if {$picturefile == ""} {return}
	if {![file exists $reportExcelPath]} {puts "report file not exist"; return}
	...
	if {[file exists $pictureSaveFile]} {file delete $pictureSaveFile}
	...
	if {![file exists $pictureSaveFile] && $pictureType eq "pdf"} {
		...
		# 保存为图片
		$pictureObj savePNG $pictureSaveFile -zoom 3.5
	...
	try {
		...
		set workbook [$workbooks Open $reportExcelPath]
		...
		for {set i 0} {$i<[llength $pictureNmaeList]} {incr i} {
			...
			$shape AddPicture [file nativename [file normalize [lindex $pictureNmaeList $i]]] 0 1 $xlocation 100 -1 -1
			incr xlocation [expr $width-429]
			$cropedImg close
...
proc videoCapture {} {
    ...
    set outVideo [file join [pwd] videos $name.avi]
...
proc openPDF {{filename ""}} {
	global PDFhandle pages testProcedurePath
	if {$filename eq ""} {
		set filename [tk_getOpenFile]
	}
	if {$filename eq ""} {return}

	set testProcedurePath $filename
	
	if {[info exist PDFhandle]} {
		# catch {$PDFhandle close}
		closePDF
	}
	set PDFhandle [mupdf::open $testProcedurePath]
...
# 关闭右上角的测试流程PDF文件查看器
proc closePDF {} {
	global PDFhandle annotIDLocation testProcedurePath
...
# 在右上角的测试流程图中画“√”或者“×”
proc drawSymbol {var} {
	global PDFhandle x y annotIDLocation
	...
	# with modified version of muPdfWidget
	catch {.main.frame.testframe.c refreshpage}
...
# 在右上角的测试流程图中清除“√”或者“×”
proc clearSymbol {} {
	global PDFhandle x y annotIDLocation testProcedurePath
	variable currentPage

	set pageHandle [$PDFhandle getpage $currentPage]
	if {![info exist annotIDLocation] || $annotIDLocation eq ""} {
		set filename [file join [pwd] data "annotIDLocation_[lindex [split [file tail $testProcedurePath] .] 0].txt"]
		set file [open $filename r]
		set annotIDLocation [read $file]
		close $file
	}

	dict for {key value} $annotIDLocation {
		...
		if {[lindex $key end] eq "ID" && [lindex $coords_page end] eq $currentPage} {
			...
			if {$distance < 60} {
				...
				foreach annotID $value {
					catch {$pageHandle annot $annotID delete}
				}

				# 清空文本的记录
				dict unset annotIDLocation $key
				dict unset annotIDLocation [lreplace $key end end Time]
				break
			}
		}
	}
...
proc updatecomments {commentsarear} {
	global commentsPath testProcedurePath
	set commentsPath [file join [pwd] data "comments_[lindex [split [file tail $testProcedurePath] .] 0].txt"]
...
proc readReportTemplate {} {
	global allTitles reportExcelPath
	set allTitles ""

	set app [Excel OpenNew false]
	set wb [Excel OpenWorkbook $app $reportExcelPath]
...
proc getFunctionResult {text} {
	global titleDataRange testProcedurePath
		...
		if {$title eq $text} {
			...
			dict for {key value} $fileData {
				...
				if {[lindex $coords_page end] <= $page2 && [lindex $coords_page end] >= $page1} {
					...
					if {[lindex $coords_page end] > $page1 && [lindex $coords_page end] < $page2 || [lindex $coords_page end] == $page1 && [lindex $coords 1] > $coords1 || [lindex $coords_page end] == $page2 && [lindex $coords 1] < $coords2} {
...
proc fillReport {} {
	global titleDataRange reportExcelPath schemePlanPath reportStartCell
	saveTitlePos
	set app [Excel OpenNew false]
	set name [file tail $reportExcelPath]
	set time [clock format [clock second] -format %y-%m-%d-%H-%M]
    set outDoc [file join [pwd] report $time-$name]
    file copy -force $reportExcelPath $outDoc
	...
	set row [string index $reportStartCell 1]
	foreach {key value} $titleDataRange {
		set res [getFunctionResult $key]
		Excel SetCellValue $ws $row [Excel ColumnCharToInt [string index $reportStartCell 0]] $res
		incr row
	}
